...
export async function createCustomerOrder(
  data: Omit<CustomerOrder, "id" | "createdAt" | "totalAmount"> & { id?: string; createdAt?: string }
...
export async function updateCustomerOrder(
  ...
  if (updateData.items) {
    updateData.totalAmount = (updateData.items || []).reduce((acc: number, item: any) => acc + item.quantity * item.price, 0);
  }
